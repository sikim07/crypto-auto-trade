...
/**
 * ══════════════════════════════════════════════════════════════════
 * [v3.8.20260320] 폭락 종목 절대 차단선 — 방향가중만으로는 폭락 종목 반복 선정되는 구조적 문제 해결
 * ══════════════════════════════════════════════════════════════════
 *
 * [배경]
 *   v3.7(방향가중) 적용 이후에도 KRW-BARD(-43%)가 반복 선정되어 3회 연속 손절 발생(2026-03-18~20).
 *   원인: DOWNWARD_WEIGHT=0.5이지만 하락폭이 클수록 점수가 여전히 올라가는 구조.
 *     BARD -43%: 43 × 0.5 = 21.5점
 *     AKT  +10%: 10 × 1.5 = 15.0점
 *     → 폭락 종목이 상승 종목보다 높은 점수로 1위 선정됨.
 *   방향가중은 "동일 변동폭에서 상승 종목 우선"이지, 폭락 규모에 대한 절대 상한이 아님.
 *
 * [목적]
 ...
 *
 * [주요 값]
 *   SELECT_MAX_DOWNWARD_RATE = 0.15 (15%)
 *     - 하루 -15% 이상 하락한 종목은 구조적 매도 압력이 높아 스캘핑 전략의 손익비 불리.
 *     - 10%로 낮추면 선정 풀이 지나치게 좁아질 수 있어 15%로 보수 설정.
 *     - 시장 전반 하락장에서 후보가 0개가 되는 경우는 re-select 로그로 모니터링 필요.
 *
 * [앞으로 확인할 것]
 *   - "[selectMarkets] 거래대금 상위 N개" 로그에서 필터 후 후보가 0~1개가 되는 빈도 체크.
 *     자주 발생하면 0.20으로 완화 검토.
 *   - BARD류 폭락 종목 차단 이후 대체 선정 종목의 손익 비교.
 *   - 15% 차단이 과도해 유효한 반등 종목을 놓친다면 10%로 타이트하게 조정.
 * ══════════════════════════════════════════════════════════════════
 */
/** 종목 선정 최대 하락률 필터 — 24h 등락률이 이 값 이상 하락한 종목은 후보에서 완전 제외 */
export const SELECT_MAX_DOWNWARD_RATE = 0.15;
