...
export const calculateRSI = (
  ...
  /**
   * ──────────────────────────────────────────────────────────────
   * [v3.5.20260315] avgGain=0 & avgLoss=0 엣지케이스 수정
   *
   * [수정 이유]
   *   기존 코드: avgLoss === 0 이면 무조건 RSI=100 반환.
   *   그러나 avgGain도 0인 경우(가격 변동 없는 극단적 평탄 시장)에는
   *   RS = 0/0 (정의 불가)이며, RSI는 중립값 50이 수학적으로 올바름.
   *   RSI=100이 반환되면 RSI 60/70 상향 돌파 조건을 즉시 충족해
   *   전략 B·D에서 허위 매수 신호가 발생할 수 있음.
   *
   * [변경 내용]
   *   avgLoss === 0 && avgGain === 0 → RSI=50 (중립) 반환
   *   avgLoss === 0 && avgGain > 0   → RSI=100 (기존 동작 유지, 수학적으로 정확)
   *
   * [앞으로 확인할 것]
   *   - 실전에서 avgGain+avgLoss=0 케이스가 로그에 나타나는지 모니터링.
   *   - 1분봉 암호화폐 시장에서 발생 빈도는 극히 낮으나, 거래 정지·상장폐지
   *     직전 종목에서 발생 가능.
   * ──────────────────────────────────────────────────────────────
   */
